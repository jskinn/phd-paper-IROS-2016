...
Using high-fidelity simulation has allowed us to explore the behaviour of SeqSLAM more comprehensively than ever before, and has revealed new details of it's behaviour. Further investigation will be required to find the reason for this, which will lead to it becoming more reliable and even more robust.
...
% Niko, uncomment this when/if you get the object recognition stuff done.
...
\textbf{Benefits of Simulation:} The performance curves generated using simulated datasets for SeqSLAM show a peculiar behaviour for certain parameter values. This enables a comprehensive evaluation of the algorithm for such cases which is not expected in general. The corresponding input parameters of SeqSLAM which might have caused the unexpected intermittent performance drop as shown in the Figure \ref{fig:seqslam-results-stacked} could be the \emph{Offset Matching Range} and/or \emph{Patch Normalization} factor. We further analysed the performance drop for different times of day at $10\degree$ pitch. Figure shows the SAD matrix comparison with ground truth, true positives and false positives marked.

\begin{figure*}[t]
    \includegraphics[scale=0.4]{result-marked-dawn-down-10}
    \includegraphics[scale=0.4]{result-marked-morning-down-10}
    \includegraphics[scale=0.4]{result-marked-noon-down-10}
    \includegraphics[scale=0.4]{result-marked-afternoon-down-10}
    \includegraphics[scale=0.4]{result-marked-sunset-down-10}
    \caption{The SAD matrix for comparison of Noon Baseline dataset with a change in $10\degree$ pitch for increasing time of day from dawn to sunset. The ground truth is marked in blue and forms the diagonal of the matrix with true positives overlaid in green on the diagonal. The false positives are marked in red.}
...
\begin{figure*}[t]
    \includegraphics[scale=0.25]{real-data-lateralOffset}
    \caption{Images for five different traversals of a street with different lateral offsets. This real world data is used to compare the trend of performance change with change in lateral shifts as compared to the simulated data.}
...
\bibliography{john-bibliography,sourav-visual-SLAM}  
